...
[AddComponentMenu("Physics 2D Toolkit/Triple Explosion", order: 3)]
public class TripleExplosion : UniversalExplosion
{
    ...
    public float firstExplosionForce = 15f;
    ...
    [Tooltip("The force of the explosion.")]
    public float secondExplosionForce = 35f;
    ...
    [Space(1)]
    [Header("Third Explosion Settings")]
    [DrawRectWithColor(3f, 0f, 1f, 0f, 0.7f)]
    [Tooltip("Delay of the third explosion (In seconds).")]
    public float thirdExplosionDelay = 0.55f;
    [Space(6)]

    [Tooltip("Radius of the circle within witch the explosion has its effect.")]
    public float thirdExplosionRadius = 4.5f;
    [Tooltip("The explosion position offset.")]
    public Vector3 thirdExplosionOffset = default(Vector3);
    [Tooltip("The force of the explosion.")]
    public float thirdExplosionForce = 60f;

    [Space(3)]
    [Tooltip("if set to true, Force will be modified by distance,NOTE! that bodies with the center of mass outside the radius will not be affected by the explosion!")]
    public bool thirdExplosionModifyForceByDistance = true;
    [Tooltip("The method used to apply the force to its targets.")]
    public ForceMode2D thirdExplosionForceMode = ForceMode2D.Impulse;
    ...
    [Space(5)]
    public UnityEvent onThirdExplosion = default(UnityEvent);

    private bool calculateTimers = false;
    private bool secondExplosionOccoured = false;
    private bool thirdExplosionOccoured = false;
    private float secondExplosionDelayCounterTimer = 0f;
    private float thirdExplosionDelayCounterTimer = 0f;
    ...
    private void SecondExplosion()
    {
        ...
        secondExplosionOccoured = true;
        if (thirdExplosionOccoured)
            Finish();
    }
    ...
    private void ThirdExplosion()
    {
        if (useNonAllocation)
        {
            ThirdExplosionNonAlloc();
            return;
        }
        colliders = Physics2D.OverlapCircleAll(_transform.position + thirdExplosionOffset, thirdExplosionRadius, layerFilter, minDepth, maxDepth);
        foreach (Collider2D hit in colliders)
        {
            Rigidbody2D Rb;
            if (useAttachedRigidbody)
            ...
            if (CheckCollider(hit) && CheckCollidedRigidbody2D(Rb))
            {
                Rb.AddExplosionForce2D(thirdExplosionForce, _transform.position + thirdExplosionOffset, thirdExplosionRadius, thirdExplosionModifyForceByDistance, thirdExplosionForceMode);
                if (sendThirdExplosionDamage)
                    SendThirdExplosionDamage(Rb.gameObject);
            }
        }
        if (onThirdExplosion != null)
            onThirdExplosion.Invoke();
        thirdExplosionOccoured = true;
        if (secondExplosionOccoured)
            Finish();
    }
    ...
    protected override void UpdateCalculations()
    {
        if (calculateTimers)
        {
            ...
            if (!thirdExplosionOccoured)
            {
                thirdExplosionDelayCounterTimer += Time.deltaTime;
                if (thirdExplosionDelayCounterTimer >= thirdExplosionDelay)
                    ThirdExplosion();
            }
        }
    }
    ...
    protected override void Reset(bool deactivate = false)
    {
        ...
        thirdExplosionDelayCounterTimer = 0f;
        secondExplosionDelayCounterTimer = 0f;
        secondExplosionOccoured = false;
        thirdExplosionOccoured = false;
    ...
    [Tooltip("if set to true, 'Explosion Damage' will be modified based on the distance between rigidbodies and the explosion center.")]
    [BoolConditionalHide("sendSecondExplosionDamage", true, false)] public bool secondModifyDamageByDistance = true;
    ...
    {
        if (secondModifyDamageByDistance)
        {
    ...
    [Tooltip("Send the custom explosion damage.")]
    public bool sendThirdExplosionDamage = false;

    [Space(6)]

    [Tooltip("This is the custom damage and has nothing to do with explosion force. You can specify this, for example, based on enemy type.")]
    [BoolConditionalHide("sendThirdExplosionDamage", true, false)] public float thirdExplosionDamage = 0f;

    [Tooltip("if set to true, 'Explosion Damage' will be modified based on the distance between rigidbodies and the explosion center.")]
    [BoolConditionalHide("sendThirdExplosionDamage", true, false)] public bool thirdModifyDamageByDistance = true;

    [Space(6)]

    [Tooltip("The name of the method to call.")]
    [BoolConditionalHide("sendThirdExplosionDamage", true, false)] public string thirdMethodToCall;

    [Tooltip("Should an error be raised if the method doesn't exist on the target object?")]
    [BoolConditionalHide("sendThirdExplosionDamage", true, false)] public SendMessageOptions thirdOptions = SendMessageOptions.DontRequireReceiver;

    private float thirdFinalExplosionDamage = 0f;
    private float thirdDamageByDistanceModifier = 0f;

    private void SendThirdExplosionDamage(GameObject toObject)
    {
        if (thirdModifyDamageByDistance)
        {
            thirdDamageByDistanceModifier = 1.0f - (Vector2.Distance(_transform.position + thirdExplosionOffset, toObject.transform.position) / thirdExplosionRadius);
            if (thirdDamageByDistanceModifier > 0f)
                thirdFinalExplosionDamage = thirdExplosionDamage * thirdDamageByDistanceModifier;
            else
                return;
        }
        else
        {
            thirdFinalExplosionDamage = thirdExplosionDamage;
        }
        toObject.SendMessage(thirdMethodToCall, thirdFinalExplosionDamage, thirdOptions);
    }
    ...
    // Third Explosion :
    // Make sure to comment the existing ThirdExplosion() method!
    private void ThirdExplosionNonAlloc()
    {
        ...
        // Then we get all colliders that fall within circular area.
        Physics2D.OverlapCircleNonAlloc(_transform.position + thirdExplosionOffset, thirdExplosionRadius, collidersNonAlloc, layerFilter, minDepth, maxDepth);
        ...
        foreach (Collider2D hit in collidersNonAlloc)
        {
            ...
            { // We check if Collider2D exist (Is not null)
                ...
                if (CheckCollider(hit) && CheckCollidedRigidbody2D(Rb))
                { // And if Rigidbody2D pass the filter check
                    Rb.AddExplosionForce2D(thirdExplosionForce, _transform.position + thirdExplosionOffset, thirdExplosionRadius, thirdExplosionModifyForceByDistance, thirdExplosionForceMode); // We Add Explosion Force.
                    if (sendThirdExplosionDamage)
                        SendThirdExplosionDamage(Rb.gameObject);
                }
            }
        ...
        if (onThirdExplosion != null)
            onThirdExplosion.Invoke(); // Invoke event.
