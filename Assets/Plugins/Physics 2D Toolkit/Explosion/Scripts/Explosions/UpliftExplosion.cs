...
[AddComponentMenu("Physics 2D Toolkit/Uplift Explosion", order: 1)]
public class UpliftExplosion : UniversalExplosion
{
    ...
    public override void Activate()
    {
        ...
        foreach (Collider2D hit in colliders)
        {
            ...
            {
                Rb.AddUpliftedExplosionForce2D(explosionForce, _transform.position + explosionOffset, explosionRadius, upliftModifier, modifyForceByDistance, forceMode);
        ...
        foreach (Collider2D hit in collidersNonAlloc)
        {
            ...
            { // We check if Collider2D exist (Is not null)
                ...
                { // And if Rigidbody2D pass the filter check
                    Rb.AddUpliftedExplosionForce2D(explosionForce, _transform.position + explosionOffset, explosionRadius, upliftModifier, modifyForceByDistance, forceMode); // We Add Explosion Force.
                    if (sendExplosionDamage)
                        SendExplosionDamage(Rb.gameObject);
                }
            }
