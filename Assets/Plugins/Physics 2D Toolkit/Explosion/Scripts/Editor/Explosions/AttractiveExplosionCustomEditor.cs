...
[CustomEditor(typeof(AttractiveExplosion))]
public class AttractiveExplosionCustomEditor : ExplosionUniversalEditor {

	AttractiveExplosion scriptRef;

	void OnEnable () {
		scriptRef = (AttractiveExplosion)target;
	...
	void OnSceneGUI () {
		...
		Handles.DrawWireDisc (objectTransform.position + scriptRef.attractionOffset, Vector3.forward, scriptRef.attractionRadius);
	...
	private void ScaleRadiusHandler () {
		...
		var centerPosition_1 = scriptRef.transform.position + scriptRef.attractionOffset;
		var centerPosition_2 = scriptRef.transform.position + scriptRef.explosionOffset;

		var handlesPos = new [] {
			new Vector3(centerPosition_1.x + scriptRef.attractionRadius, centerPosition_1.y, 0f),
		...
		if (Event.current.type == EventType.MouseDown && Event.current.button == 0 || Event.current.type == EventType.MouseUp && Event.current.button == 0) {
			capturedRadius_1 = scriptRef.attractionRadius;
		...
		float size_1 = HandleUtility.GetHandleSize(scriptRef.transform.position + scriptRef.attractionOffset) * 0.1f;
		...
		if (EditorGUI.EndChangeCheck()) {
			...
			if (capturedRadius_1 + offset <= 0f)
				scriptRef.attractionRadius = 0f;
			else
				scriptRef.attractionRadius = capturedRadius_1 + offset;
		}
		...
		if (ExplosionForce2DPreferences.showTextAboveHandles) {
			radiusText = Math.Round (scriptRef.attractionRadius, 2).ToString ();
